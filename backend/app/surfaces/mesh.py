...
def cache_key(
    cube_hash: str, field: str, orbital: int | None, spin: str, sign: str, isovalue: float
) -> str:
    payload = json.dumps(
        [MESH_CACHE_VERSION, cube_hash, field, orbital, spin, sign, round(isovalue, 9)],
        separators=(",", ":"),
    )
...
def contour_to_ply(cube: CubeData, isovalue: float, output: Path) -> None:
    ...
    boundary_max = float(np.max(np.abs(boundary_values)))
    if level <= boundary_max:
        raise ValueError(
...
def demo_surface_ply(output: Path, *, field: str, sign: int = 1, orbital: int = 0) -> None:
    ...
    centers = (
        [(0.0, 0.0, 0.0)] if field == "total_density" else [((1 if sign > 0 else -1) * 0.65, 0, 0)]
    )
